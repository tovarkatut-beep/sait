...
3. **✏️ Fix docs / write a blog / fix a typo** — _typo fix, dead link, use-case writeup_ ↳ branch `3c`
...
**3b.7** Render `templates/PR-BODY-i18n.md` with `{{DOC_NAME}}`, `{{LONG_DISPLAY_NAME}}`, `{{LANG_CODE}}`, `{{TRANSLATED_PATH}}`, `{{ENGLISH_PATH}}`, `{{STATUS}}`, `{{TRANSLATION_NOTES}}` (one paragraph from the agent: anything tricky, untranslated terms it kept, etc.), `{{DISCORD_INVITE}}`.
...
2. **I have a specific fix in mind** (free-text — what's the issue?)
...
```bash
gh api "repos/${TARGET_REPO}/contents/.github/ISSUE_TEMPLATE/bug-report.yml" \
  --jq .content | base64 -d > /tmp/od-bug-report.yml
```
...
| `version` | "Which OD version are you running? (About me menu, or `od --version`)" |
...
**3d.4** Dedup: extract 3–5 keywords from the description, run:
